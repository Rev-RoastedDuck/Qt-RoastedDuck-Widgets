...
class SlideshowWidget(QFrame):
    ...
    def __animationStartPre(self, is_forward: bool) -> None:
        ...
        if is_forward:
            ...
            # 更新导航栏
            self.nav.changeColor_signal.emit((self.imageListIndex_m + 1) % label_count)
        elif not is_forward:
    ...
    def __updatePixmap(self, is_forward: bool = True) -> None:
        ...
        if is_forward:
            ...
            self.imageList = self.imageList[1:] + [self.imageList[0]]
            self.labelList[(self.mLabelListIndex + 1) % label_count].setPixmap(self.imageList[2])
        else:
            self.imageListIndex_m = (self.imageListIndex_m - 1) % image_count
            self.imageList = [self.imageList[-1]] + self.imageList[:-1]
            self.labelList[(self.mLabelListIndex - 1) % label_count].setPixmap(self.imageList[2])
