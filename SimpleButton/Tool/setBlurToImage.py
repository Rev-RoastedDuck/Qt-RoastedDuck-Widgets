...
def increaseBrightness(image: QImage, value: int) -> QImage:
    """增加图像的亮度"""
    if image.isNull():
        return QImage()
    ...
    for y in range(height):
        for x in range(width):
            ...
            r = min(pixel.red() + value, 255)
            g = min(pixel.green() + value, 255)
            b = min(pixel.blue() + value, 255)

            new_pixel = QColor(r, g, b)

            image.setPixel(x, y, new_pixel.rgba())
