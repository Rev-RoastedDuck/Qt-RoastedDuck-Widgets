...
class AlertWidget(WidgetAnimationBase):
    def __init__(self, parent=None):
        super().__init__(parent=parent)
        self.message = ""
        self.__status = AlertWidgetStatus.Warning
    ...
    def __drawtipIcon(self, painter: QPainter, color: ColorConfig):
        match self.status:
            case AlertWidgetStatus.Succeed:
                self.__drawHook(painter, color)
            case AlertWidgetStatus.Warning:
                self.__drawExclamation(painter, color)
            case AlertWidgetStatus.Dangerous:
    ...
    def __getColor(self):
        if self.status == AlertWidgetStatus.Succeed:
            ...
            return color_config
        elif self.status == AlertWidgetStatus.Dangerous:
            color_config: ColorConfig = {
                "hook_color": QColor(240, 240, 240),
            ...
            return color_config
        elif self.status == AlertWidgetStatus.Warning:
            color_config: ColorConfig = {
                "hook_color": QColor(240, 240, 240),
    ...
    @status.setter
    def status(self, status:AlertWidgetStatus):
